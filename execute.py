...
tf.flags.DEFINE_string("filter_sizes", "1,2,3", "filter size of cnn")
...
tf.flags.DEFINE_integer("rnn_size", 200, "the number of filter in every layer")
tf.flags.DEFINE_integer("num_rnn_layers", 1, "the number of filter in every layer")
...
valid_ori_quests, valid_cand_quests, valid_labels, valid_results = load_test_data(FLAGS.valid_file, word2idx, FLAGS.sequence_len)
...
with tf.Graph().as_default():
    with tf.device("/gpu:3"):
        ...
        with tf.Session(config=session_conf).as_default() as sess:
            cnn = CNN_LSTM(FLAGS.batch_size, FLAGS.sequence_len, embedding, FLAGS.embedding_size, filter_sizes, FLAGS.num_filters, FLAGS.rnn_size, FLAGS.num_rnn_layers)
            ...
            for ori_train, cand_train, neg_train in batch_iter(ori_quests, cand_quests, FLAGS.batch_size, FLAGS.epoches):
                ...
                if cur_step % FLAGS.evaluate_every == 0 and cur_step != 0:
                    ...
                    valid_model(sess, cnn, valid_ori_quests, valid_cand_quests, valid_labels, valid_results)
